...
L'instruction \textt{break} permet de stopper l'éxécution de la boucle, c'est-à-dire de sortir de la boucle comme si la condition de sortie était remplie, ou le nombre maximal d'itérations atteint.
...
Une autre structure de contrôle intéréssant est \textt{next}, qui permet de sauter une itération si la valeur de l'itérateur ne nous plaît pas.
...
On a vu dans les séances précédentes l'existence de variables de type booléen, qui prennent les valeurs \texttt{TRUE} ou \textt{FALSE}.
...
L'opérateur répété une seule fois est \emph{elemt-wise}: les éléments des vecteurs sont comparés deux à deux (et on applique du recyclage).
L'opérateur répété deux fois 
...
l'appel à la fonction \texttt{f} devrait modifier \texttt{a}, puisque la seule instruction de cette fonction est \textt{a = a+1}.
